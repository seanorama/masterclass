# Lab notes for the Operations Masterclass
...
Wait for all nodes to report status "Success" and all host checks to pass (explore the check results to see what is done). Click next.
...
Ensure nothing is ticked on the master nodes.
...
At the Review stage, click Deploy and then watch the magic happen!
...
sudo su hdfs -l -c 'hadoop jar /usr/hdp/current/hadoop-mapreduce-client/hadoop-mapreduce-client-jobclient-tests.jar TestDFSIO -read -write -nrFiles 5 -fileSize 1GB'
...
Then run the commands as shown in the ambari screen (also shown below for convenience)
...
Once that's done repeat the Click Add Property step for the next key/value.
